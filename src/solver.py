...
def count_neighbors(grid, row, col):
    ...
    for i in range(-1, 2):
        for j in range(-1,2):
            if ((i == 0 and j == 0) or (row + i) < 0 or (col + j) < 0 or (row + i) >= len(grid) 
                or (col + j) >= len(grid[0])):
                continue

            alive_count += grid[row+i][col+j]
...
def compute_next_generation(grid):
    ...
    for i in range(0,rows):
        for j in range(0, cols):
            neighborNo = count_neighbors(grid,i,j)

            if (grid[i][j] == 1):
                if (neighborNo < 2):
                    s = 0
                elif (neighborNo == 2 or neighborNo == 3):
                    s = 1
                elif (neighborNo > 3):
                    s = 0
            else:
